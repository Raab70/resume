...
__author__ = [
    ...
    'Rob Harrigan <https://unsupervisedpandas.com>',
]
...
class RenderContext(object):
    ...
    def __init__(self, context_name, file_ending, jinja_options, replacements, order_field):
    ...
    def render(self, yaml_data):
        ...
        for section_tag, section_title in yaml_data[self._order_field]:
            ...
            elif section_tag == 'objective':
                if self._file_ending == '.md':
                    continue
                section_template_name = "section" + self._file_ending
                section_data['data'] = section_content
...
class ResumeRenderContext(RenderContext):
    BUILD_DIR = 'build'
    TEMPLATES_DIR = 'templates'
    SECTIONS_DIR = 'sections'
    DEFAULT_SECTION = 'items'
...
LATEX_RESUME_CONTEXT = ResumeRenderContext(
    'latex',
    '.tex',
    dict(
        block_start_string='~<',
...
MARKDOWN_RESUME_CONTEXT = ResumeRenderContext(
    'markdown',
    '.md',
    dict(
        trim_blocks=True,
...
def process(context, yaml_data, preview):
    rendered = context.render(yaml_data)
    if preview:
        print(rendered)
    else:
        context.write_to_outfile(rendered)


def shift(key):
    return ((0x0000FFFF & key) << 16) + ((0xFFFF0000 & key) >> 16)
...
def main():
    ...
    obf_phone = [393216, 0, 196608, 458752, 65536, 262144, 589824, 196608, 262144, 327680]
    phone = [str(shift(d)) for d in obf_phone]
    yaml_data['phone'] = f"""({"".join(phone[0:3])}) {"".join(phone[3:6])}-{"".join(phone[6:])}"""

    if args.latex or args.markdown:
        if args.latex:
            process(LATEX_CONTEXT, yaml_data, args.preview)
            process(LATEX_RESUME_CONTEXT, yaml_data, args.preview)
        elif args.markdown:
            process(MARKDOWN_CONTEXT, yaml_data, args.preview)
            process(MARKDOWN_RESUME_CONTEXT, yaml_data, args.preview)
    else:
        process(LATEX_CONTEXT, yaml_data, args.preview)
        process(LATEX_RESUME_CONTEXT, yaml_data, args.preview)
        process(MARKDOWN_CONTEXT, yaml_data, args.preview)
